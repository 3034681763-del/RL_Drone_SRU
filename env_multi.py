...
class Env(BaseEnv):
    ...
    # The old precomputed easy maps contain eight cylinder trunks in a
    # 10 m x 16 m tile. Keep that spatial density as the online-map reference.
    OLD_EASY_TRUNK_COUNT = 8
    OLD_EASY_TILE_AREA = 10.0 * 16.0
    OLD_EASY_OBSTACLE_COUNT_SCALE = 0.5
    ...
    def __init__(
        ...
        easy_density_multiplier_min=1.0,
        easy_density_multiplier_max=1.0,
        hard_density_multiplier_min=1.0,
        hard_density_multiplier_max=2.0,
        ...
        self.expanded_online_random_map = bool(compact_two_zone_map) and not bool(unified_four_maps)
        self.map_y_half = 8.0 if self.compact_two_zone_map else 12.0
        self.map_y_min = -self.map_y_half
        self.map_y_max = self.map_y_half
        core_width = self.map_x_max - self.map_x_min
        core_length = self.map_y_max - self.map_y_min
        self.outer_x_margin = 0.5 * core_width if self.expanded_online_random_map else 0.0
        self.outer_y_margin = 0.5 * core_length if self.expanded_online_random_map else 0.0
        self.obstacle_x_min = self.map_x_min - self.outer_x_margin
        self.obstacle_x_max = self.map_x_max + self.outer_x_margin
        self.obstacle_y_min = self.map_y_min - self.outer_y_margin
        self.obstacle_y_max = self.map_y_max + self.outer_y_margin
        self.map_z_max = 5.0
        self.ground_z = 0.0
        ...
        self.easy_density_multiplier_min = float(easy_density_multiplier_min)
        self.easy_density_multiplier_max = float(easy_density_multiplier_max)
        self.hard_density_multiplier_min = float(hard_density_multiplier_min)
        self.hard_density_multiplier_max = float(hard_density_multiplier_max)
        if self.easy_density_multiplier_min <= 0.0:
            raise ValueError("easy_density_multiplier_min must be positive")
        if self.easy_density_multiplier_max < self.easy_density_multiplier_min:
            raise ValueError("easy_density_multiplier_max must be >= easy_density_multiplier_min")
        if self.hard_density_multiplier_min <= 0.0:
            raise ValueError("hard_density_multiplier_min must be positive")
        if self.hard_density_multiplier_max < self.hard_density_multiplier_min:
            raise ValueError("hard_density_multiplier_max must be >= hard_density_multiplier_min")
        self.current_easy_density_multiplier = self.easy_density_multiplier_min
        self.current_hard_density_multiplier = self.hard_density_multiplier_min
    ...
    def _effective_reference_density(self, difficulty):
        """Density relative to the old precomputed easy-map spatial density."""
        global_scale = self.obstacle_count_scale / self.OLD_EASY_OBSTACLE_COUNT_SCALE
        if difficulty == "easy":
            return global_scale * self.easy_density_scale * self.current_easy_density_multiplier
        return global_scale * self.hard_density_scale * self.current_hard_density_multiplier

    def _reference_trunk_count(self, x0, x1, y0, y1, difficulty):
        area = max(0.0, float(x1 - x0)) * max(0.0, float(y1 - y0))
        return self._reference_trunk_count_for_area(area, difficulty)

    def _reference_trunk_count_for_area(self, area, difficulty):
        trunks_per_square_metre = self.OLD_EASY_TRUNK_COUNT / self.OLD_EASY_TILE_AREA
        return max(1, int(round(float(area) * trunks_per_square_metre * self._effective_reference_density(difficulty))))
    ...
    def _build_boundary_voxels(self):
        ...
        floor_x_center = 0.5 * (self.obstacle_x_min + self.obstacle_x_max)
        floor_y_center = 0.5 * (self.obstacle_y_min + self.obstacle_y_max)
        floor_x_half = 0.5 * (self.obstacle_x_max - self.obstacle_x_min)
        floor_y_half = 0.5 * (self.obstacle_y_max - self.obstacle_y_min)
        return [
            [floor_x_center, floor_y_center, 0.0, floor_x_half, floor_y_half, self.boundary_half],
        ]
    ...
    def _attach_objects_to_cylinders(
        self,
        placed,
        bounds,
        mode="scatter",
        difficulty="hard",
        reference_easy_profile=False,
        reference_ball_count=None,
        reference_box_count=None,
    ):
        ...
        elif reference_easy_profile:
            make_spec = self._make_scatter_spec
            # Match the old easy tile composition: 8 trunks, 3 balls, 2 boxes.
            # The hard region changes density, not obstacle-type composition.
            ball_count = (
                int(reference_ball_count)
                if reference_ball_count is not None
                else int(round(len(trunks) * 3.0 / 8.0))
            )
            box_count = (
                int(reference_box_count)
                if reference_box_count is not None
                else int(round(len(trunks) * 2.0 / 8.0))
            )
        else:
            make_spec = self._make_scatter_spec
            if difficulty == "easy":
    ...
    def _generate_reference_density_rectangle(
        self,
        difficulty,
        x0,
        x1,
        ...
        y1,
        *,
        preserve_core_passage=False,
        trunk_count=None,
        ball_count=None,
        box_count=None,
    ):
        """Generate one independent rectangle at the old easy spatial density."""
        edge_clearance = 0.20
        y_clearance = self.blank_length + edge_clearance if preserve_core_passage else edge_clearance
        bounds = {
            "x_lo": float(x0) + edge_clearance,
            "x_hi": float(x1) - edge_clearance,
            "y_lo": float(y0) + y_clearance,
            "y_hi": float(y1) - y_clearance,
        }
        if trunk_count is None:
            trunk_count = self._reference_trunk_count(x0, x1, y0, y1, difficulty)
        trunk_count = int(trunk_count)
        if trunk_count <= 0:
            return [], [], []
        pair_gap = 0.24
        attempts = 18 if preserve_core_passage else 1
        best_placed = None
        best_score = -1.0

        for _ in range(attempts):
            specs = [self._make_scatter_spec("cyl") for _ in range(trunk_count)]
            placed = self._place_scatter_specs(specs, bounds, pair_gap)
            placed = self._attach_objects_to_cylinders(
                placed,
                bounds,
                mode="scatter",
                difficulty=difficulty,
                reference_easy_profile=True,
                reference_ball_count=ball_count,
                reference_box_count=box_count,
            )
            if not preserve_core_passage:
                return self._packed_obstacle_lists(placed)

            occ = self._build_scatter_nav_grid(
                placed,
                y0,
                y1,
                resolution=0.35,
                inflate=0.5 * self.two_drone_passage_width,
            )
            connected = self._easy_nav_has_connection(occ)
            score = (1.0 if connected else 0.0) - self._blocked_ratio(occ)
            if score > best_score:
                best_score = score
                best_placed = placed
            if connected:
        ...
        return self._packed_obstacle_lists(best_placed or [])

    def _generate_expanded_region(self, difficulty, y0, y1, is_lower_region):
        """Build one core region plus an independently sampled outer obstacle ring."""
        rectangles = [
            (self.map_x_min, self.map_x_max, y0, y1, True),
            (self.obstacle_x_min, self.map_x_min, y0, y1, False),
            (self.map_x_max, self.obstacle_x_max, y0, y1, False),
        ]
        if is_lower_region:
            rectangles.append(
                (self.obstacle_x_min, self.obstacle_x_max, self.obstacle_y_min, self.map_y_min, False)
            )
        else:
            rectangles.append(
                (self.obstacle_x_min, self.obstacle_x_max, self.map_y_max, self.obstacle_y_max, False)
            )

        areas = [max(0.0, (x1 - x0) * (rect_y1 - rect_y0)) for x0, x1, rect_y0, rect_y1, _ in rectangles]
        total_area = sum(areas)
        total_trunks = self._reference_trunk_count_for_area(total_area, difficulty)
        raw_counts = [total_trunks * area / max(total_area, 1e-9) for area in areas]
        trunk_counts = [int(math.floor(value)) for value in raw_counts]
        remainder_order = sorted(
            range(len(raw_counts)),
            key=lambda idx: raw_counts[idx] - trunk_counts[idx],
            reverse=True,
        )
        for idx in remainder_order[:total_trunks - sum(trunk_counts)]:
            trunk_counts[idx] += 1

        def _allocate_by_trunks(total_count):
            raw = [total_count * count / max(total_trunks, 1) for count in trunk_counts]
            ...
            for idx in order[:total_count - sum(allocated)]:
                allocated[idx] += 1
            return allocated

        ball_counts = _allocate_by_trunks(int(round(total_trunks * 3.0 / 8.0)))
        box_counts = _allocate_by_trunks(int(round(total_trunks * 2.0 / 8.0)))

        balls = []
        cyls = []
        voxels = []
        for rect_idx, ((x0, x1, rect_y0, rect_y1, preserve_core_passage), trunk_count) in enumerate(
            zip(rectangles, trunk_counts)
        ):
            rect_balls, rect_cyls, rect_voxels = self._generate_reference_density_rectangle(
                difficulty,
                x0,
                x1,
                rect_y0,
                rect_y1,
                preserve_core_passage=preserve_core_passage,
                trunk_count=trunk_count,
                ball_count=ball_counts[rect_idx],
                box_count=box_counts[rect_idx],
            )
            balls.extend(rect_balls)
            cyls.extend(rect_cyls)
            voxels.extend(rect_voxels)
    ...
    def reset(self):
        ...
        self.max_speed = float(5.0 * self.speed_mtp)
        ...
        # One easy and one hard multiplier are sampled per regenerated map.
        # Keeping them common across the batch preserves rectangular obstacle
        # tensors while every batch item still gets independent coordinates.
        self.current_easy_density_multiplier = random.uniform(
            self.easy_density_multiplier_min,
            self.easy_density_multiplier_max,
        )
        self.current_hard_density_multiplier = random.uniform(
            self.hard_density_multiplier_min,
            self.hard_density_multiplier_max,
        )
        ...
        for _ in range(B):
            ...
            for slot_idx, region_type in enumerate(order):
                ...
                if region_type == "easy":
                    if self.expanded_online_random_map:
                        region_balls, region_cyls, region_voxels = self._generate_expanded_region(
                            "easy", region_y0, region_y1, is_lower_region=(slot_idx == 0)
                        )
                    else:
                    ...
                    if self.expanded_online_random_map:
                        region_balls, region_cyls, region_voxels = self._generate_expanded_region(
                            "hard", region_y0, region_y1, is_lower_region=(slot_idx == 0)
                        )
                    else:
            ...
            u_meta_batch.append(u_meta or {
                "open_left": None,
                "exit_side": "unknown",
                "easy_density_multiplier": self.current_easy_density_multiplier,
                "hard_density_multiplier": self.current_hard_density_multiplier,
                "obstacle_bounds": (
                    self.obstacle_x_min,
                    self.obstacle_x_max,
                    self.obstacle_y_min,
