...
def _tile_easy_geometry_xy(geom: Dict, tile_radius: int = 1) -> Dict:
    """Replicate an easy map in an XY square while keeping center spawn points fixed."""
    radius = max(0, int(tile_radius))
    if radius == 0:
        return geom

    center_x_min = float(geom.get("map_x_min", 0.0))
    center_x_max = float(geom["map_x_max"])
    center_y_min = float(geom["map_y_min"])
    center_y_max = float(geom["map_y_max"])
    tile_x_period = center_x_max - center_x_min
    tile_y_period = center_y_max - center_y_min
    if tile_x_period <= 0.0 or tile_y_period <= 0.0:
        raise ValueError(
            f"invalid center-map span for XY tiling: x={tile_x_period}, y={tile_y_period}"
        )

    cyl_h = _ensure_np(geom.get("cyl_h", []), 3)
    if cyl_h.shape[0] > 0:
        raise ValueError("XY tiling does not support horizontal cylinders without a Y center coordinate")

    offsets = [
        (ix * tile_x_period, iy * tile_y_period)
        for iy in range(-radius, radius + 1)
        for ix in range(-radius, radius + 1)
    ]

    def _tile_xy(arr, cols):
        base = _ensure_np(arr, cols)
        if base.shape[0] == 0:
            return base
        copies = []
        ...
        return np.concatenate(copies, axis=0).astype(np.float32, copy=False)

    tiled = dict(geom)
    tiled["balls"] = _tile_xy(geom.get("balls", []), 4)
    tiled["cyl"] = _tile_xy(geom.get("cyl", []), 3)
    tiled["voxels"] = _tile_xy(geom.get("voxels", []), 6)
    tiled["cyl_h"] = cyl_h
    tiled["map_x_min"] = center_x_min - radius * tile_x_period
    tiled["map_x_max"] = center_x_max + radius * tile_x_period
    tiled["map_y_min"] = center_y_min - radius * tile_y_period
    tiled["map_y_max"] = center_y_max + radius * tile_y_period
    tiled["map_length"] = tiled["map_y_max"] - tiled["map_y_min"]

    map_meta = dict(geom.get("map_meta", {}))
    side = 2 * radius + 1
    map_meta.update({
        "xy_tiling": f"{side}x{side}",
        "xy_tile_radius": radius,
        "tile_x_period": tile_x_period,
        "tile_y_period": tile_y_period,
        "center_map_x_min": center_x_min,
        "center_map_x_max": center_x_max,
        "center_map_y_min": center_y_min,
        "center_map_y_max": center_y_max,
    })
...
def _build_single_map_unified(task: Dict):
    ...
    try:
        ...
        if map_type == "easy":
            geom = _tile_easy_geometry_xy(
                geom,
                tile_radius=int(task.get("easy_xy_tile_radius", 0)),
            )
    ...
    for map_type in MAP_TYPE_ORDER:
        ...
        for local_idx in range(local_total):
            ...
            tasks.append({
                ...
                "easy_density_scale": float(args.easy_density_scale),
                "hard_density_scale": float(args.hard_density_scale),
                "easy_xy_tile_radius": int(args.easy_xy_tile_radius),
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Replicate each easy map in XY; 1 creates a 3x3 layout with eight surrounding copies",
    )
    parser.add_argument(
        "--num_workers",
        type=int,
