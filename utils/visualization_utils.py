...
def draw_cylinder_z(ax, cx, cy, r, z0, z1, color='royalblue', alpha=0.14, res_theta=18, res_h=2):
...
def draw_cylinder_y(ax, cx, zc, r, y0, y1, color='royalblue', alpha=0.16, res_theta=18, res_h=2):
    theta = np.linspace(0.0, 2.0 * np.pi, res_theta)
...
def _plotly_add_cylinder_z(fig, cx, cy, r, z0, z1, color='royalblue', opacity=0.72, res_theta=22):
...
def _plotly_add_cylinder_y(fig, cx, zc, r, y0, y1, color='royalblue', opacity=0.72, res_theta=22):
    th = np.linspace(0.0, 2.0 * np.pi, res_theta)
    y = np.array([y0, y1])
...
def save_interactive_3d_html(html_path, env, p_cpu, v_cpu, R_cpu=None, idx=0, axis_len=0.3, axis_step=5,
    ...
    if hasattr(env, 'cyl') and env.cyl.numel() > 0:
        ...
        for cx, cy, cr in cyl[:100]:
            _plotly_add_cylinder_z(fig, float(cx), float(cy), float(cr), z0_vis, z1_vis, color='royalblue', opacity=0.76)
    ...
    if hasattr(env, 'cyl_h') and env.cyl_h.numel() > 0:
        ...
        for cx, cz, cr in cyl_h[:100]:
            _plotly_add_cylinder_y(fig, float(cx), float(cz), float(cr), y0_vis, y1_vis, color='royalblue', opacity=0.76)
            x_vals.extend([[cx - cr], [cx + cr]])
...
def save_cached_viz_record(
    ...
    *,
    ...
    w_cpu = record.get('weights_cpu', None)
    ...
    if getattr(args, 'use_precomputed_potential_maps', False) and potential_map_cache is not None and map_idx >= 0:
    ...
    if w_cpu is not None and not is_best and is_debug_tb_step(save_step, args):
        labels = [
            'AvoidanceWeight', 'ExplorationWeight', 'TurnWeight',
            'ProgressWeight', 'SmoothnessWeight', 'EnergyWeight',
        ]
        tag_suffix = [
            '0_AvoidanceWeight', '1_ExplorationWeight', '2_TurnWeight',
            '3_ProgressWeight', '4_SmoothnessWeight', '5_EnergyWeight',
        ]
