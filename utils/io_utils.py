...
def sync_multi_pub_to_checkpoint_dir(save_dir):
    ...
    # When experiment outputs live inside the workspace, exclude their
    # top-level directory from the source mirror.  Otherwise the destination
    # snapshot would recursively copy itself on subsequent runs.
    save_dir_abs = os.path.abspath(save_dir)
    if os.path.commonpath([src_root, save_dir_abs]) == src_root:
        save_rel = os.path.relpath(save_dir_abs, src_root)
        if save_rel != os.curdir:
            ignore_names.add(save_rel.split(os.sep, 1)[0])
...
def load_compatible_checkpoint(
    ...
    output_row_mappings=None,
    force_output_row_mapping=False,
):
    if not path:
        return
    ...
    for key, value in state_dict.items():
        ...
        row_indices = output_row_indices
        if output_row_mappings is not None:
            row_indices = output_row_mappings.get(value.shape[0], row_indices)
        force_row_mapping = bool(force_output_row_mapping and row_indices is not None)
        if value.shape == target_value.shape and not force_row_mapping:
            compatible_state[key] = value
            continue
        valid_row_indices = (
            [source_row for source_row in row_indices if source_row is not None]
            if row_indices is not None else []
        )
        if (
            row_indices is not None
            and value.dim() == target_value.dim()
            and value.dim() >= 1
            and target_value.shape[0] == len(row_indices)
            and value.shape[1:] == target_value.shape[1:]
            and max(valid_row_indices, default=-1) < value.shape[0]
        ):
            remapped = torch.zeros_like(target_value) if zero_expanded else target_value.clone()
            for target_row, source_row in enumerate(row_indices):
                if source_row is not None:
                    remapped[target_row] = value[source_row]
            compatible_state[key] = remapped
